...
public final class FxMath {
    ...
    static Point2D normalizeVector(double x, double y) {
        double length = Math.sqrt(
                FastMath.pow(Math.abs(x), 2) + Math.pow(Math.abs(y), 2)
        );
        return new Point2D(
                x / length,
                y / length);
     ...
     */
    static Point2D vec;
    public static Point2D attractiveForce(Point2D from, Point2D to, int numVertices, double force, double scale, double distance) {
    ...
    static double attractiveFunction(double distance, int numVertices, double force, double scale) {
        ...
        return force * FastMath.log(distance / scale) * 0.5 * (1 / (stabilizer2 * numVertices));
    }
