...
import static DSI_Graph_Main_Controlers.DSI_Model.duration;
import static DSI_Graph_Main_Controlers.DSI_Model.proccesRunning;

public class TesterLayoutDSI_norm {
...
//    }
    ...
    public TesterLayoutDSI_norm RunThread(String variant, String reference, String version, String TesterName, String TesterID, int Columns, int Rows, ObservableList<SystemPintable> systemPintableXML) {
...
//                            //System.out.println(mounted);
                            if (!mounted.equals("truefalse")) {
                                if (rs.getString(27).equals(TesterName)) {
                                    ...
                                    double finalI = i2;
                                    Platform.runLater(() -> {indicator.setProgress((finalI));mainProgressBar.setProgress(0.3+(finalI/1000));});
                                }
                            }
                        }
...
//                                            });
                        ...
                        double finalI = i;
                        Platform.runLater(() -> {indicator.setProgress((finalI));mainProgressBar.setProgress(0.4+(finalI/1000));});
                        iSP++;
                    }
        ...
        Task task_4 = new Task(){
            ...
            protected Object call() {/**        wyszukiwanie najmniejszych wspó³¿ednych,         Rysowanie Konektorów            ********/
                try{
                    ...
                    for (Section4_Model section4_model1 : DSI_Model.getSection4_models()) {
                        if (section4_model1.getVariant().contains(Asembly.get()) || Asembly.get().equals("_Full")) {
                            ...
                            Platform.runLater(() -> {indicator.setProgress((finalSc4inc));mainProgressBar.setProgress(0.5+(finalSc4inc/1000));});
                        }
               ...
               // });
                   ...
                   // DSI_Model.getSection4_models().parallelStream().forEach(section4_model -> {
                    for (Section4_Model section4_model : DSI_Model.getSection4_models()) {
                        boolean firstPointAnother = true;
                        boolean secondPointAnother = true;
...
//                            model.addCell(section4_model.getSecond_Point(), CellType. CONNECTOR, Double.parseDouble(section4_model.getCoordinates2().split("y")[0].replace("x","")),Double.parseDouble(section4_model.getCoordinates2().split("y")[1]));
                                                ...
                                                if (section6_model_DrwConn.getComp_Type().equals("CONNECTOR")) {
                                                    ...
                                                    for (Cell cell : model.getAddedCells()) {
                                                        if (cell.getCellId().equals(section6_model_DrwConn.getRef_BSJ())) {
                                                            Cell pasiveCell = new Cell(section6_model_DrwConn.getRef_BSJ(), CellType.COMPONENT.name(), section6_model_DrwConn.getName(), section6_model_DrwConn.getPart_Nb(), "", "0", Arrays.asList(variants.clone()));
                                                    ...
                                                    for (Cell cell : model.getAddedCells()) {
                                                        if (cell.getCellId().equals(section6_model_DrwConn.getRef_BSJ())) {
                                                            Cell pasiveCell = new Cell("Cav: "+section6_model_DrwConn.getCavity()+" / "+section6_model_DrwConn.getComp_Type(), CellType.COMPONENT.name(), section6_model_DrwConn.getName(), section6_model_DrwConn.getPart_Nb(), "", section6_model_DrwConn.getComp_Type(), Arrays.asList(variants.clone()));
                                                            pasiveCell.getPositionXProperty().bind(cell.getPositionXProperty());
                                                            pasiveCell.getPositionYProperty().bind(cell.getPositionYProperty());
                                                            cell.addPasives(pasiveCell);
                                                        }
                        ...
                        Platform.runLater(() -> {indicator.setProgress((finalSc4inc));mainProgressBar.setProgress(0.6+(finalSc4inc/1000));});
                    }
                   // });
...
//                                            model.addEdge(section4_model.getFirst_Point(), section4_model.getSecond_Point(), Double.parseDouble(section4_model.getLenght()), 5, Color.TEAL);
                            ...
                            double finalSc4inc = sc4inc;
                            Platform.runLater(() -> {indicator.setProgress((finalSc4inc));mainProgressBar.setProgress(0.9+(finalSc4inc/1000));});
                        }
                    }
