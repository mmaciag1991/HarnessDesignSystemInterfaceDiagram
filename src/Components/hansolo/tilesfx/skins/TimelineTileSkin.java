...
import Components.hansolo.tilesfx.tools.DoubleExponentialSmoothingForLinearSeries.Model;
...
import static Components.hansolo.tilesfx.tools.Helper.enableNode;
...
public class TimelineTileSkin extends TileSkin {
    ...
    @Override protected void handleCurrentValue(final double VALUE) {
        ...
        long minTime = now.minus(timePeriod.getSeconds(), ChronoUnit.SECONDS).getEpochSecond();
    ...
    private int getNoOfVerticalLines(final Instant START, final Duration TIME_PERIOD) {
        ...
        long minTime    = START.minus(TIME_PERIOD.getSeconds(), ChronoUnit.SECONDS).getEpochSecond();
        int  lineCountX = 0;
        ZonedDateTime dateTime;
        for (long t = minTime ;  t < maxTime ; t++) {
            dateTime = ZonedDateTime.ofInstant(Instant.ofEpochSecond(t), tile.getZoneId());
    ...
    @Override protected void resizeDynamicText() {
        ...
        percentageInSections.entrySet().forEach(entry -> {
            ...
            entry.getValue().relocate(size * 0.05, sections.get(entry.getKey()).getLayoutBounds().getDepth() - entry.getValue().getLayoutBounds().getDepth());
