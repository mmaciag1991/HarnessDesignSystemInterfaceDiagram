 ...
 */
package Components.hansolo.tilesfx.tools;
...
public class MatrixIcon {
    ...
    public String getCode() {
        StringBuilder code = new StringBuilder().append("MatrixIcon resources.icon = new MatrixIcon();").append("\n");
        int startIndex       = -1;
        int endIndex         = -1;
            ...
            for (int x = 0 ; x < 8 ; x++) {
                ...
                if (currentPixelColor.equals(nextPixelColor)) {
                    ...
                    if (currentPixelColor.equals(lastPixelColor)) {
                        if (startIndex > -1) {
                            endIndex = x;
                            code.append("resources.icon.fillPixels(").append(startIndex).append(", ").append(endIndex).append(", ").append(y).append(", Color.web(\"").append(currentPixelColor.toString().replace("0x", "#")).append("\"));\n");
                        } else {
                            code.append("resources.icon.setPixel(").append(x).append(", ").append(y).append(", Color.web(\"").append(currentPixelColor.toString().replace("0x", "#")).append("\"));\n");
                        }
                    } else {
                        code.append("resources.icon.setPixel(").append(x).append(", ").append(y).append(", Color.web(\"").append(currentPixelColor.toString().replace("0x", "#")).append("\"));\n");
                    }
